  ...
  #startSidrPlugin = () => {
    ...
    sidr.new(".mobile-menu", {
      ...
      onOpen() {
        ...
        document.querySelector("a.sidr-class-toggle-sidr-close svg")?.classList.remove("sidr-class-hvc-icon", "sidr-class-hvc-icon--close");
        document.querySelector("a.sidr-class-toggle-sidr-close svg")?.classList.add("hvc-icon", "hvc-icon--close");

        self.#elements.hamburgerBtn?.classList.add("is-active");

        self.#elements.header.insertAdjacentHTML(
          "afterend",
          '<div class="havocwp-sidr-overlay" tabindex=0></div>'
        );

        const overlay = document.querySelector(".havocwp-sidr-overlay");
...
havocwp.sidebarMobileMenu = new SidebarMobileMenu();
