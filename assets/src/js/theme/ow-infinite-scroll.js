    ...
    this.#infiniteScroll.on("load", function (body, path, response) {
      ...
      imagesLoaded(items, () => {
        ...
        if (this.element.classList.contains("blog-masonry-grid")) {
          havocwp?.blogMasonry?.isotop.appended(items);

          // Fix Gallery posts
          if (!!this.element.querySelector(".gallery-format")) {
            setTimeout(() => {
              havocwp?.blogMasonry?.isotop.layout();
            }, 600 + 1);
          }
        }
        ...
        if (!document.body.classList.contains("no-lightbox")) {
          havocwp?.owLightbox?.initSingleImageLightbox();
          havocwp?.owLightbox?.initGalleryLightbox();
        }
...
document.addEventListener("DOMContentLoaded", () => {
  havocwp.owInfiniteScroll = new OWInfiniteScroll();
});
