...
class WooQuickView {
  ...
  #setElements = () => {
    this.#elements = {
      ...
      body: document.body,
      modal: document.querySelector("#hvc-qv-wrap"),
      content: document.querySelector("#hvc-qv-content"),
    };
  };
  ...
  #setupEventListeners = () => {
    delegate(".hvc-quick-view", "click", this.#onQuickViewBtnClick);

    document
      .querySelectorAll(".hvc-qv-overlay, .hvc-qv-close")
      .forEach((closeBtn) => {
    ...
    delegate(
      document.body,
      "#hvc-qv-content .product:not(.product-type-external) .single_add_to_cart_button",
      "click",
  ...
  #onAddToCartBtnClick = (event) => {
    ...
    if (formData.some(({ name }) => name === "add-to-cart")) {
      ...
      jQuery.ajax({
        type: "POST",
        url: havocwpLocalize.wc_ajax_url,
        data: formData,
  ...
  #open = (quickViewBtn, productId) => {
    axios
      .post(
        options.ajax_url,
        qs.stringify({
          action: "havocwp_product_quick_view",
          nonce: options.nonce,
      ...
      .then(({ data }) => {
        ...
        this.#elements.body.classList.add("hvc-qv-open");
        this.#elements.content.innerHTML = data.output;

        // Run quantity button
        havocwpWooCustomFeatures.quantityButtons.start();
        ...
        const galleryImagesWrapper =
          this.#elements.content.querySelector(".hvc-qv-image");
  ...
  #close = () => {
    if (visible(this.#elements.modal)) {
      ...
      this.#elements.html.style.marginRight = "";
      this.#elements.html.classList.remove("hvc-qv-open");
