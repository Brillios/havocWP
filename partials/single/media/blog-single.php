...
// LLMS Integration.
if ( HAVOCWP_LIFTERLMS_ACTIVE ) {
	$details = get_theme_mod( 'havoc_llms_course_details', array( 'image', 'description', 'meta', 'author', 'progress', 'syllabus' ) );

	if ( is_course() && ! in_array( 'image', $details, true ) ) {
		return;
	}

	if ( is_membership() && ! ( get_theme_mod( 'havoc_llms_membership_image', false ) ) ) {
		return;
	}
}

// Define image alt text usage status.
$sp_seo_set = get_theme_mod( 'havoc_enable_sp_fimage_alt', false );
...
$sp_fimage_alt = ( false === $sp_seo_set || ( true === $sp_seo_set && ! $spfe_img_alt ) ) ? esc_attr( havocwp_theme_strings( 'hvc-string-current-read', false ) ) . ' ' . get_the_title() : esc_attr( $spfe_img_alt );
