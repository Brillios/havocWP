...
const SearchBoxModal = ( { ...props } ) => {
    ...
    const switchThemeMode = () => {
        ...
        havocCustomizerSearchOptions.darkMode = havocCustomizerSearchOptions.darkMode === '1'
        || havocCustomizerSearchOptions.darkMode === 'true'
        || havocCustomizerSearchOptions.darkMode === true ? false : true;
        jQuery( '.modal-dialog.havoc-customize-search-modal' ).toggleClass( 'light-theme' );

        wp.ajax.post( 'havoc_update_search_box_light_mode', {
            darkMode: havocCustomizerSearchOptions.darkMode
        } );
    }
    ...
    const ClickHandler = ( sectionName, ElementID ) => {
        ...
        section.expand();
        setsearchResults( '' );
        jQuery( '.havoc-customize-search-modal' ).parent().fadeOut();
        jQuery( '.modal-backdrop.show' ).removeClass( 'show' );
        props.onHide();

        setTimeout( function() {
            jQuery( '.havoc-customize-search-modal .modal-header button.close' ).trigger( 'click' ).trigger( 'mouseup' )
            if ( jQuery( '#customize-control-' + ElementID ).length ) {
                ...
                jQuery( '#customize-control-' + ElementID ).addClass( 'havoc-control-focused' );
            }
        }, 1500 )

        setTimeout( () => {
            jQuery( '.havoc-control-focused' ).removeClass( 'havoc-control-focused' );
        }, 8000 );
    }
    ...
    return <>
        <Modal
            ...
            onHide = { props.onHide }
            dialogClassName = {
                havocCustomizerSearchOptions.darkMode === '1'
                || havocCustomizerSearchOptions.darkMode === 'true'
                || havocCustomizerSearchOptions.darkMode === true ? "havoc-customize-search-modal" : "havoc-customize-search-modal light-theme" }
            aria-labelledby = "contained-modal-title-vcenter"
            centered>
            <Modal.Header closeButton>
                <Form.Group className="full-width" controlId="havoc-wp-customize-search-input">
