jQuery(document).on('havocready', function() {
	var $ = jQuery;
	"use strict";

	// Update the values for all our input fields and initialise the sortable repeater
	$('.sortable_repeater_control:not(.havoc-proccessed)').each(function() {
		$(this).addClass('havoc-proccessed');
	...
	$(this).find('.sortable_repeater.sortable:not(.havoc-proccessed)').sortable({
		update: function(event, ui) {
			skyrocketGetAllInputs($(this).parent());
		}
	});

	$(this).find('.sortable_repeater.sortable:not(.havoc-proccessed)').addClass('havoc-proccessed');


	// Remove item starting from it's parent element
	$('.sortable_repeater.sortable:not(.havoc-proccessed)').on('click', '.customize-control-sortable-repeater-delete', function(event) {
		$(this).addClass('havoc-proccessed');
	...
	// Add new item
	$('.customize-control-sortable-repeater-add:not(.havoc-proccessed)').click(function(event) {
		$(this).addClass('havoc-proccessed');
		event.preventDefault();
	...
	$('.sortable_repeater.sortable:not(.havoc-proccessed)').change(function() {
		$(this).addClass('havoc-proccessed');
		skyrocketGetAllInputs($(this).parent());
	})

	// Add https:// to the start of the URL if it doesn't have it
	$('.sortable_repeater.sortable:not(.havoc-proccessed)').on('blur', '.repeater-input', function() {
		$(this).addClass('havoc-proccessed');
		var url = $(this);
		var val = url.val();
		if (val && !val.match(/^.+:\/\/.*/)) {
	...
	$('.slider-custom-control:not(.havoc-proccessed)').each(function() {
		$(this).addClass('havoc-proccessed');
	...
	$('.slider:not(.havoc-proccessed)').on('slide', function(event, ui) {
		$(this).addClass('havoc-proccessed');
		$(this).parent().find('.customize-control-slider-value').val(ui.value);
	});

	// Reset slider and input field back to the default value
	$('.slider-reset:not(.havoc-proccessed)').on('click', function() {
		$(this).addClass('havoc-proccessed');
	...
	$('.single-accordion-toggle:not(.havoc-proccessed)').click(function() {
		$(this).addClass('havoc-proccessed');
	...
	$('.multi-image-checkbox:not(.havoc-proccessed)').on('change', function() {
		$(this).addClass('havoc-proccessed');
	...
	$(".pill_checkbox_control .sortable:not(.havoc-proccessed)").sortable({
		placeholder: "pill-ui-state-highlight",
		update: function(event, ui) {
			skyrocketGetAllPillCheckboxes($(this).parent());
		}
	});
	$(".pill_checkbox_control .sortable:not(.havoc-proccessed)").addClass('havoc-proccessed');

	$('.pill_checkbox_control .sortable-pill-checkbox:not(.havoc-proccessed)').on('change', function() {
		$(this).addClass('havoc-proccessed');
	...
	$('.customize-control-dropdown-select2:not(.havoc-proccessed)').each(function() {
		$(this).addClass('havoc-proccessed');
	...
	$(".customize-control-select2:not(.havoc-proccessed)").on("change", function() {
		$(this).addClass('havoc-proccessed');
	...
	$('.google-fonts-list:not(.havoc-proccessed)').each(function(i, obj) {
		$(this).addClass('havoc-proccessed');
	...
	$('.google-fonts-list:not(.havoc-proccessed)').on('change', function() {
		$(this).addClass('havoc-proccessed');
	...
	$('.google_fonts_select_control select:not(.havoc-proccessed)').on('change', function() {
		$(this).addClass('havoc-proccessed');
		skyrocketGetAllSelects($(this).parent().parent());
	});
	...
	$('.customize-control-tinymce-editor:not(.havoc-proccessed)').each(function() {
		$(this).addClass('havoc-proccessed');
	...
	$('.wpcolorpicker-alpha-color-picker:not(.havoc-proccessed)').each(function(i, obj) {
		$(this).addClass('havoc-proccessed');
	...
	$('.alpha-color-control:not(.havoc-proccessed)').each(function() {
		...
		colorPickerOptions = {
			...
			palettes: havocwpLocalize.colorPalettes // Use the passed in palette.
		};
