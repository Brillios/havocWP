...
// Start and run class.
if ( ! class_exists( 'HavocWP_Gutenberg_Editor' ) ) {

	/**
	 * HavocWP Gutenberg Class
	 */
	class HavocWP_Gutenberg_Editor {
		...
		public static function elements() {
			...
			return apply_filters(
				'havoc_gutenberg_typography_tags',
				array(
					'body'            => array(
		...
		public static function loop( $return = 'css' ) {
			...
			if ( ! empty( $elements ) ) {
				...
				if ( 'css' === $return && ! empty( $css ) ) {
					$css = '/* HavocWP Gutenberg Style */' . $css;
					return $css;
				}
		...
		public function gutenberg_editor_style( $output ) {
			...
			if ( ! is_null( get_current_screen() ) && get_current_screen()->is_block_editor() ) {
				...
				$heading_tag = get_theme_mod( 'havoc_single_post_heading_tag', 'h2' );
				...
				$background_layout           = get_theme_mod( 'havoc_main_layout_style', 'wide' );
				$background_color            = get_theme_mod( 'havoc_background_color', '#ffffff' );
				$boxed_separate_bg_color     = get_theme_mod( 'havoc_boxed_inner_bg', '#ffffff' );
				...
				if ( ! empty( $gutenberg_css ) ) {
					echo '<style id="havoc-gutenberg-style"> ' . esc_attr( $gutenberg_css ) . '</style>';
				}
			}
...
return new HavocWP_Gutenberg_Editor();
